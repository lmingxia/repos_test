...
export SCRIPT_NAME=schedule_procedure
...
export log_file_name=${SCRIPT_NAME}${DB_SERVICE}.log
echo $log_file_name


CMD="${ORACLE_HOME}/bin/sqlplus ${BI_DB_USER}/'"${BI_DB_PSWD}"'@${DB_SERVICE} @${SCRIPT_NAME}.sql $DB_SERVICE"
echo "Running: "
echo $CMD
eval "$CMD"
retVal=$?

echo "Entering Error Check..."
...
mail -s "${DB_SERVICE} - ReportsError" -a "schedule_procedure_${DB_SERVICE}.log" Mingxia.Luo@va.gov
#retVal=55
#exit $retVal
fi
