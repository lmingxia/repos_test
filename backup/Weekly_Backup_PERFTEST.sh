...
cd /tmp
CMD="${ORACLE_HOME}/bin/expdp $SRC_USER/$SRC_PW@//${DB_NODE_IP//[[:blank:]]/}:1521/${DB_SERVICE} directory=VA_DMP_DIR dumpfile=${DB_SERVICE}_${EXPORTED_DATA_DAY}_Weekly_Backup_%u.dmp logfile=${DB_SERVICE}_${EXPORTED_DATA_DAY}_Nightly_Backup.log FULL=Y parallel=4 exclude=statistics reuse_dumpfiles=Y compression=DATA_ONLY exclude=COMMENT FILESIZE=10GB"
