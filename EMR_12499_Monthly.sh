...
export SCRIPT_NAME=EMR_12499_Monthly
...
CMD="${ORACLE_HOME}/bin/sqlplus ${PB_DB_USER}/'"${PB_DB_PSWD}"'@${DB_SERVICE} @${SCRIPT_NAME}.sql ${DB_SERVICE}"
echo "Running: "
echo $CMD
eval "$CMD"
retVal=$?
...
echo "Subject: EMR_12499 Failed -- ${DB_SERVICE}" | /usr/sbin/sendmail patrick.burns2@va.gov
...
echo "Attached report is for the previous month." | mail -s "PB Monthly Login Report" -a "${SCRIPT_NAME}.log" kurt.shriner@va.gov,patrick.burns2@va.gov,kayla.evans@va.gov,james.lahey@va.gov,Scott.Helmer@va.gov,Matthew.Brown26@va.gov,Kurt.shriner@va.gov,Ben.Truong@va.gov,Yogesh.Bhaskarwar@va.gov,Youssef.Kaouk@va.gov,Miguel.Archundia@va.gov,Scott.Kelley3@va.gov
